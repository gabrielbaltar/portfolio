...
export function BlogPostPage() {
      ...
      <div className="min-h-screen flex items-center justify-center font-['Inter',sans-serif] px-5" style={{ backgroundColor: "var(--bg-primary, #0B0B0D)" }}>
        ...
        >
          ...
          <p className="mb-6" style={{ fontSize: "14px", lineHeight: "22px", color: "var(--text-secondary, #888)" }}>
            Este artigo contem conteudo exclusivo. Digite a senha para acessar.
          </p>
          ...
          >
            <ArrowLeft size={13} /> Voltar ao blog
          </Link>
        </motion.div>
      </div>
    );
  ...
  // Metadata
  const meta = [
    post.author ? { label: "Autor", value: post.author, icon: User } : null,
    post.date ? { label: "Data", value: post.date, icon: Clock } : null,
    post.readTime ? { label: "Leitura", value: post.readTime, icon: Clock } : null,
    post.category ? { label: "Categoria", value: post.category, icon: Tag } : null,
    post.services ? { label: "Topicos", value: post.services, icon: Tag } : null,
    ...
    >
      ...
      <div className="max-w-[700px] mx-auto px-5 pt-24 md:pt-28">
        ...
        {post.link && post.link.trim() !== "" && (
          ...
          >
            {t("visitWebsite") || "Ver publicacao original"} <ExternalLink size={12} />
          </motion.a>
        )}
