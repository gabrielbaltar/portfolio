...
export function getVersions(contentId: string): ContentVersion[] {
  const result = dataProvider.loadVersions(contentId);
...
export function saveVersion(contentId: string, data: Record<string, any>, label: string) {
  return dataProvider.saveVersion(contentId, data, label);
}
      ...
      {Array.from({ length: maxLen }).map((_, i) => {
        ...
        return (
          <div key={i} className="rounded-md overflow-hidden" style={{ border: "1px solid #1e1e1e" }}>
            ...
            <div className="grid grid-cols-2 divide-x" style={{ divideColor: "#1e1e1e" }}>
              {/* Old */}
...
export function VersionHistoryPanel({ contentId, currentData, onRestore, onClose }: {
  ...
  const [versions, setVersions] = useState<ContentVersion[]>(() => getVersions(contentId));
    ...
    return (
      <div className="fixed inset-0 z-50 flex">
        ...
        <div className="relative ml-auto w-full max-w-[700px] h-full overflow-hidden flex flex-col" style={{ backgroundColor: "#0e0e0e" }}>
          ...
          {/* Version labels */}
          <div className="grid grid-cols-2 divide-x shrink-0" style={{ divideColor: "#1e1e1e", borderBottom: "1px solid #1e1e1e" }}>
